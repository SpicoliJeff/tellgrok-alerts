    ...
    try {
      ...
      if (highSells >= 3 || manipulation) {
        await resend.emails.send({
          from: 'TellGrok Alerts <onboarding@resend.dev>', // Use this default until you verify a domain
          to: sub.email,
          subject: `🚨 Alert: Activity on ${sub.symbol || sub.address.slice(0,8)}...`,
          text: `Detected ${highSells >= 3 ? `${highSells} large sells in 20min` : ''}${highSells >= 3 && manipulation ? ' and ' : ''}${manipulation ? 'similar amount patterns' : ''}.\n\nCheck your token now!`
        });
      }
    } catch (e) {
